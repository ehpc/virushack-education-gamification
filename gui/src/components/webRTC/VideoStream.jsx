...
/** CONFIG * */
const SIGNALING_SERVER = window.location.protocol + '//' + window.location.host;
...
export default class TeacherRTC extends Component {
    ...
    function init() {
      ...
      signaling_socket.on('addPeer', (config) => {
        ...
        peer_connection.onaddstream = function (event) {
          ...
          window.attachMediaStream(remote_media[0], event.stream);
        };
    ...
    function setup_local_media(callback, errorback) {
      ...
      navigator.getUserMedia({ audio: USE_AUDIO, video: USE_VIDEO },
        (stream) => { /* user accepted access to a/v */
          ...
          window.attachMediaStream(local_media[0], stream);
  ...
  render() {
    return (
      <div id="videos">
        {/* <video id="localVideo" autoPlay muted ref={(video) => (this.localVideo = video)} /> */}
      </div>
    );
  }
}
